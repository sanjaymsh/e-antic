...
int main(int argc, char* const argv[])
{
    ...
    /* x^3 - 2 */
    fmpq_poly_init(p);
    fmpq_poly_set_coeff_si(p, 3, 1);
    fmpq_poly_set_coeff_si(p, 0, -2);

    arb_init(emb);
    arb_set_d(emb, 1.2599210498948732);
    ...
    /* x = 13 a^2 + 230 a - 2 */
    fmpq_poly_zero(p);
    fmpq_poly_set_coeff_si(p, 2, 13);
    fmpq_poly_set_coeff_si(p, 1, 230);
    fmpq_poly_set_coeff_si(p, 0, -2);
    renf_elem_set_fmpq_poly(x, p, nf);

    /* y = -53 a^2 + 134 a + 120*/
    fmpq_poly_zero(p);
    fmpq_poly_set_coeff_si(p, 2, -53);
    fmpq_poly_set_coeff_si(p, 1, -134);
